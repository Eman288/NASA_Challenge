...
// stars from Nasa file*************************************************************
// Create star shape function
const color = new THREE.Color(0xFDFFD3); // Make sure this is a valid hex
...
// Load star data from stars.json and create star meshes
async function loadStarData() {
  try {
    const response = await fetch("normStarsjson.json");
    const data = await response.json(); // Assuming the file is in JSON format and has an array of stars

    data.forEach(star => {
      const x = star.x + 100; // Position in space
      const y = star.y + 100;
      const z = star.z + 100;

      // إنشاء الخامات اللي هتظهر النجم باستخدام اللون اللي في البيانات
      const starMaterial = new THREE.MeshStandardMaterial({
        color: star._3d.color, // Using the color from the JSON
        emissive: star._3d.color,
        emissiveIntensity: 1,
      });

      // Create star shape
      const starMesh = new THREE.Mesh(createStarShape(), starMaterial);
      
      // Set the position of the star relative to the sun
...
const pluto = new createPlanet('Pluto', 1, 350, 57, plutoTexture)
  ...
  const planetData = {
    'Mercury': {
        ...
        url: 'planets/mercury/index.html'
    },
    'Venus': {
        radius: '6,051.8 km',
        tilt: '177.4°',
    ...
    'Jupiter': {
        ...
        url: 'planets/jupiter/index.html'
    },
    'Saturn': {
        radius: '58,232 km',
        tilt: '26.73°',
    ...
    'Neptune': {
        ...
        url: 'planets/neptune/index.html'
    },
    'Pluto': {
        radius: '1,188.3 km',
        tilt: '122.53°',
...
window.addEventListener('resize', function(){
  ...
  controls.rotateSpeed = 0.5; // Control the speed of rotation, reducing it may feel smoother
